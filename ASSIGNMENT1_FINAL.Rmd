...
india_plot <- read_csv("india_data.csv") %>% 
  ...
  theme_classic() +
  ggtitle(expression(paste("      Actual vs. Calculated Per Capita
                           Rate of Change in India 1963-2004",  (frac(dN, Ndt)))))
...
country_plots <- ggplot(pop_data_mills_df, aes(x = pop_mills, y= dNNdt)) +
  ...
  labs(y = expression(paste("Per Capita Rate of Change ", (frac(dN, Ndt)))), x = "Population Size (N~t~, Millions)") +
  theme_classic() +
...
$\frac{dN}{Ndt}\ $ = $2.975*10^{-2}$ + $1.292*10^{-7}$ (Calories (Person/Day)) - $2.95*10^{-2}$ (Years Educated), education level is a significant predicator of per capita population growth rate (p<0.001).  However, the food consumption, defined as the number of calories eaten per person, per day does not have a significant (p=0.549) effect on $\frac{dN}{Ndt}\ $

All things other factors remaining constant, for each unit increase in education level, $\frac{dN}{Ndt}\ $ significantly increases by $2.95*10^{-2}$. 
...
###B. Perform a multilinear regression of education and food consumption for each countries
```{r, echo = FALSE, results = 'asis'}
##### Create Tables

regression_table <- stargazer(pop_lm1, brazil_lm1, india_lm1, japan_lm1, mexico_lm1, sk_lm1,
                      title = "Linear Regression of Education and Food Results",
                      type = "html",
...
All things other factors remaining constant, for each unit increase in education level, $\frac{dN}{Ndt}\ $ decreases by $2.420*10^{-3}$. 
...
All things other factors remaining constant, for each unit increase in education level, $\frac{dN}{Ndt}\ $ decreases by $1.523*10^{-3}$. 
...
All things other factors remaining constant, for each unit increase in education level, $\frac{dN}{Ndt}\ $ decreases by $3.834*10^{-3}$. 
All things other factors remaining constant, for each unit increase in calories per person, per day, $\frac{dN}{Ndt}\ $ decreases by $1.978*10^{-6}$. 
...
All things other factors remaining constant, for each unit increase in education level, $\frac{dN}{Ndt}\ $ decreases by $2.280*10^{-3}$. 
...
All things other factors remaining constant, for each unit increase in education level, $\frac{dN}{Ndt}\ $ decreases by $2.757*10^{-3}$. 
All things other factors remaining constant, for each unit increase in calories per person, per day, $\frac{dN}{Ndt}\ $ decreases by $4.279*10^{-7}$. 
